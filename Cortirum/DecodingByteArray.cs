...
namespace CortriumBLE.Cortrium

{
    public class ECGBatchData
    {
        ...
        public int[] ECGChannel1filtered { get; set; }
        public int[] ECGChannel2filtered { get; set; }
        public int[] ECGChannel3filtered { get; set; }
    ...
    public class DecodingByteArray
    {
        private bool firstRunAfterConnection = true;
        ...
        private int[] decoded_iECG1 = new int[12];
        private int[] decoded_iECG2 = new int[12];
        private int[] decoded_iECG3 = new int[12];
        ...
        public ECGBatchData DecodeBytes(sbyte[] data)
        {
            ...
            if (Events != 0x00)
            {
                if ((Events & 0x01) == 0x01) // Battery event
                {
                    ...
                    Console.WriteLine("Battery: " + fPercent);
            ...
            for (int i = nSamples - 1; i >= 0; i--)
            {
                ...
                {
                    System.Diagnostics.Debug.WriteLine("SW", "decode error : ( index > Length ) : " + index + " >= " + telegramLength);
                }
            ...
            var hf = new EcgHighPassFilter();
            var lf = new EcgLowPassFilter();

            for (int i = 0; i < ecgData.ECGChannel1.Length; i++)
            {
                ecgData.ECGChannel1[i] = hf.FilterInput(ecgData.ECGChannel1[i]); // Apply highpass
                ecgData.ECGChannel1[i] = lf.FilterInput(ecgData.ECGChannel1[i]); // Apply lowpass
        ...
        private Tuple<int, int> DecodeChannelDifference(int bitmask, int index, sbyte[] data, int Cx_diff)
        {
            int tmp_buf = 0;
            if (bitmask == 2)
            {
